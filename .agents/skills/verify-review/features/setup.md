...
GitHub repository choices require a signed-in account and repository access granted to the Review GitHub App. The GitHub step has `Sign in with GitHub`, `Cancel sign-in`, and `Sign out` controls. The account control at the bottom of the sidebar opens a menu. Its `Account` item returns to setup. The control shows the GitHub avatar, profile name, and public email, with username and initials when those values are unavailable. Use the local HTTP fixture for denial, cancellation, revocation, and account-switch checks. Codex sign-in opens an external browser. Do not automate credential entry. The app stores preferences in the local SQLite database through the local service.
